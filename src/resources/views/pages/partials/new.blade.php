                ...
                @foreach($new as $product)
                    ...
                    <div class="col-sm-6 col-md-3 wow zoomIn" id="featured-container">
                        <a href="{{ route('show.product', $product->product_name) }}">
                            @if ($product->photos->count() === 0)
                                <img src="./public/images/no-image-found.jpg" alt="No Image Found Tag" id="Product-similar-Image" style="width: 200px; height: 200px;">
                            @else
                    ...
                    <div class="col-sm-6 col-md-3 wow zoomIn" id="featured-container-m">
                        <a href="{{ route('show.product', $product->product_name) }}">
                            ...
                            <div class="col-xs-6">
                                @if ($product->photos->count() === 0)
                                    <img src="/newstore/src/public/images/no-image-found.jpg" alt="No Image Found Tag" id="image-m" style="width: 200px; height: 200px;">
                                @else
                                    @if ($product->featuredPhoto)
                                        <img src="/newstore/{{ $product->featuredPhoto->thumbnail_path }}" alt="Photo ID: {{ $product->featuredPhoto->id }}" class="img-responsive img-thumbnail" id="image-m" />
                                    @elseif(!$product->featuredPhoto)
                                        <img src="/newstore/{{ $product->photos->first()->thumbnail_path}}" alt="Photo" class="img-responsive img-thumbnail" id="image-m" />
                                    @else
                                        N/A
                                    @endif
                                @endif
                        ...
                        <form action="/newstore/cart/add" method="post" name="add_to_cart">
                            {!! csrf_field() !!}
                            <input type="hidden" name="product" value="{{$product->id}}" />
                            <input type="hidden" name="qty" value="1" />
                            <button class="btn btn-default waves-effect waves-light">ADD TO CART</button>
